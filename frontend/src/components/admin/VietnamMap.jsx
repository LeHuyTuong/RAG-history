const VietnamMap = ({ className = '', lat, lng, locations = [] }) => {
  // Approximate equirectangular projection calibrated for the SVG viewBox
  const getX = (lngVal) => (parseFloat(lngVal) - 102.14) * 114.93;
  const getY = (latVal) => (23.39 - parseFloat(latVal)) * 69.05;

  const validSinglePin = lat && lng && !isNaN(lat) && !isNaN(lng);

  return (
    <div
      className={`relative w-full h-full overflow-hidden ${className}`}
      style={{
        background: 'radial-gradient(circle at center, #F4EBD8 0%, #D8C3A5 100%)',
        boxShadow: 'inset 0 0 50px rgba(92, 74, 61, 0.3)'
      }}
    >
      <svg
      ...
      >
        {/* Decorative Portolan Rhumb Lines */}
        <g stroke="#8B7355" strokeWidth="1" opacity="0.3">
          <line x1="200" y1="220" x2="1024" y2="100" />
          <line x1="200" y1="220" x2="1024" y2="400" />
          <line x1="200" y1="220" x2="1024" y2="600" />
          <line x1="200" y1="220" x2="1024" y2="800" />
          <line x1="200" y1="220" x2="1024" y2="1024" />
          <line x1="200" y1="220" x2="700" y2="1024" />
          <line x1="200" y1="220" x2="400" y2="1024" />
          <line x1="200" y1="220" x2="100" y2="1024" />
        </g>

        {/* Vintage Compass Rose */}
        <g transform="translate(200, 220) scale(1)">
          <circle cx="0" cy="0" r="60" fill="none" stroke="#8b7355" strokeWidth="1" opacity="0.5" />
          <circle cx="0" cy="0" r="50" fill="none" stroke="#8b7355" strokeWidth="1.5" strokeDasharray="4 4" opacity="0.6" />
          {/* North */}
          <polygon points="0,-70 12,-15 0,0" fill="#4a3a31" />
          <polygon points="0,-70 -12,-15 0,0" fill="#8b7355" />
          {/* South */}
          <polygon points="0,70 12,15 0,0" fill="#8b7355" />
          <polygon points="0,70 -12,15 0,0" fill="#4a3a31" />
          {/* East */}
          <polygon points="70,0 15,12 0,0" fill="#4a3a31" />
          <polygon points="70,0 15,-12 0,0" fill="#8b7355" />
          {/* West */}
          <polygon points="-70,0 -15,12 0,0" fill="#8b7355" />
          <polygon points="-70,0 -15,-12 0,0" fill="#4a3a31" />
          {/* Center */}
          <circle cx="0" cy="0" r="5" fill="#e4d3b4" stroke="#4a3a31" strokeWidth="2" />
          <text x="0" y="-85" fontSize="20" fill="#4a3a31" fontWeight="bold" textAnchor="middle" style={{ fontFamily: 'Georgia, serif' }}>B</text>
          <text x="0" y="98" fontSize="18" fill="#4a3a31" fontWeight="bold" textAnchor="middle" style={{ fontFamily: 'Georgia, serif' }}>N</text>
          <text x="-85" y="6" fontSize="18" fill="#4a3a31" fontWeight="bold" textAnchor="middle" style={{ fontFamily: 'Georgia, serif' }}>T</text>
          <text x="85" y="6" fontSize="18" fill="#4a3a31" fontWeight="bold" textAnchor="middle" style={{ fontFamily: 'Georgia, serif' }}>Đ</text>
        </g>
        <g transform="translate(-100, 1024) scale(0.100000,-0.100000)">
...
-48 75 -41 33 -98 66 -113 66 -4 0 -22 -13 -38 -28z" />
        ...
        <g transform="translate(680, 480)">
          <circle cx="0" cy="0" r="4" fill="#4a3a31" />
          <circle cx="12" cy="8" r="3" fill="#4a3a31" />
          <circle cx="20" cy="-5" r="5" fill="#4a3a31" />
          <circle cx="30" cy="10" r="4" fill="#4a3a31" />

          <line x1="-15" y1="-15" x2="0" y2="0" stroke="#8b7355" strokeWidth="1.5" strokeDasharray="3 3" />
          <text x="-25" y="-20" fontSize="22" fill="#4a3a31" fontWeight="bold" textAnchor="end" style={{ fontFamily: 'Georgia, serif', textShadow: '1px 1px 0px #F4EBD8, -1px -1px 0px #F4EBD8' }}>QĐ. Hoàng Sa</text>
          <text x="-25" y="-3" fontSize="14" fill="#8b7355" textAnchor="end" style={{ fontFamily: 'Georgia, serif', fontStyle: 'italic' }}>Đại Việt</text>
        </g>

        {/* Quần đảo Trường Sa */}
        <g transform="translate(780, 800)">
          <circle cx="0" cy="0" r="3" fill="#4a3a31" />
          <circle cx="15" cy="15" r="4" fill="#4a3a31" />
          <circle cx="5" cy="25" r="3" fill="#4a3a31" />
          <circle cx="25" cy="10" r="5" fill="#4a3a31" />
          <circle cx="35" cy="30" r="4" fill="#4a3a31" />
          <circle cx="45" cy="20" r="3" fill="#4a3a31" />
          <circle cx="-10" cy="10" r="3" fill="#4a3a31" />

          <line x1="-15" y1="-15" x2="5" y2="5" stroke="#8b7355" strokeWidth="1.5" strokeDasharray="3 3" />
          <text x="-25" y="-20" fontSize="22" fill="#4a3a31" fontWeight="bold" textAnchor="end" style={{ fontFamily: 'Georgia, serif', textShadow: '1px 1px 0px #F4EBD8, -1px -1px 0px #F4EBD8' }}>QĐ. Trường Sa</text>
          <text x="-25" y="-3" fontSize="14" fill="#8b7355" textAnchor="end" style={{ fontFamily: 'Georgia, serif', fontStyle: 'italic' }}>Đại Việt</text>
        </g>

        {/* Dynamic Map Pins */}
        {validSinglePin && (
          <g transform={`translate(${getX(lng)}, ${getY(lat)})`}>
            <circle cx="0" cy="0" r="10" fill="#e11d48" className="animate-pulse" opacity="0.8" />
            <circle cx="0" cy="0" r="4" fill="#ffffff" />
            <path d="M 0 -15 L 5 -5 L 15 0 L 5 5 L 0 15 L -5 5 L -15 0 L -5 -5 Z" fill="#e11d48" opacity="0.3" className="animate-spin-slow" />
          </g>
        )}

        {locations.map((loc, idx) => {
          if (!loc.coords) return null;
          // coords format might be "Lat, Lng" string
          const [lLat, lLng] = loc.coords.split(',').map(s => s.trim());
          if (!lLat || !lLng || isNaN(lLat) || isNaN(lLng)) return null;

          return (
            <g key={idx} transform={`translate(${getX(lLng)}, ${getY(lLat)})`}>
              <circle cx="0" cy="0" r="6" fill="#e11d48" opacity="0.7" />
              <circle cx="0" cy="0" r="2" fill="#ffffff" />
            </g>
          );
        })}

        {/* Additional Historical Decals */}
        <text x="800" y="280" fontSize="36" fill="#8b7355" opacity="0.3" textAnchor="middle" style={{ fontFamily: 'Georgia, serif', fontStyle: 'italic', letterSpacing: '12px' }}>BIỂN ĐÔNG</text>
      </svg>
    </div>
  );
};
